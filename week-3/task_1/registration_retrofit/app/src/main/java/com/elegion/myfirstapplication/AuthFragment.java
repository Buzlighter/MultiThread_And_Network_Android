    ...
    private View.OnClickListener mOnEnterClickListener = new View.OnClickListener() {
        ...
        public void onClick(View view) {
            if (isEmailValid() && isPasswordValid()) {
                ...
                ApiUtils.getApiService(email, pass).authUser().enqueue(new retrofit2.Callback<User>() {
                    Handler mainHandler = new Handler(getActivity().getMainLooper());
                    @Override
                    public void onResponse(final retrofit2.Call<User> call, final retrofit2.Response<User> response) {
                            ...
                            public void run() {
                                if (!response.isSuccessful()) {
                                    ...
                                    showMessage(R.string.auth_error);
                                        ...
                                        startProfileIntent.putExtra(ProfileActivity.USER_KEY, response.body());
